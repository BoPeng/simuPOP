...
{
  m_surface = m_top_node->surface_at_point(start());
}
...
unsigned int marker_index)  const
{
  ...
  m_top_node->initialize_marker(marker_index, marker);
  std::auto_ptr<Mutator> mutator(marker.create_mutator(conf,*this));
  m_top_node->mutate_marker(marker_index,*mutator);
}
...
{
  os << "  <interval_node id=\"m_" << this << "\">" << std::endl
    << "    <child ref=\"m_" << top_node() << "\"/>" << std::endl
