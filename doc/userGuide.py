...
simu.evolve(
    ...
    preOps = [parentsTagger(infoFields='parent_idx')],
    gen = 5
)
pop = simu.extract(0)
ind = pop.individual(0)
...
simu.evolve(
    initOps = initSex(),
    preOps = [pyOperator(func=mutator, param=(10000, 2e-6))],
    gen = 200
)
for pop in simu.populations():
...
simu.evolve(
    ...
    preOps = [pause(stopOnKeyStroke=str(x), reps=x) for x in range(10)],
    gen = 100
)
#end_file
