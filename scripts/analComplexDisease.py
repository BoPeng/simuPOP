...
The results are saved in a file in python pickle format. To load it, run, for 
example:
  > import pickle
  > res = pickle.load(open('additive.res'))
  > res.keys()
  > res['DSL']
  > ...

The following variables will be saved (returned):
  FIXME: write detailed information about each variable.
  
"""

import simuOpt, simuUtil
import os, sys, types, exceptions, os.path, operator
try:
    from rpy import *
    hasRPy = True
except:
...
def drawSamples(pop, peneFunc, numSample, dataDir, reAnalyzeOnly):
    ''' get samples of different type using a penetrance function,
        and save samples in dataDir in simuPOP and linkage format
    ...
    for ns in range(numSample):
        penDir = os.path.join(dataDir, '%s%d' % (peneFunc, ns))
        if reAnalyzeOnly and os.path.isdir(penDir):
        ...
        try:
            ...
            sampleFile = os.path.join(penDir, "caseControl.txt")
            if reAnalyzeOnly:
                try:
                    # try to load previous sample
                    s = [LoadPopulation(sampleFile)]
                ...
                print "Write case-control sample in simuPOP binary format: %s" % sampleFile
                s[0].savePopulation(sampleFile)
        except Exception, err:
        ...
        try:
            ...
            sampleFile = os.path.join(penDir, "affectedSibpairs.txt")
            if reAnalyzeOnly:
                try:
                    affected = [LoadPopulation(sampleFile) ]
                except Exception, err:
                ...
                print "Write affected sibpair sample in simuPOP binary format: %s " % sampleFile
                affected[0].savePopulation(sampleFile)
                linDir = os.path.join(penDir, "Linkage")
                _mkdir(linDir)
    ...
    for sn in range(numSample):
        ...
        for ch in range(pop.numChrom()):
            if 'TDT' in mappingMethods:
                ...
                res['TDT_%d' % sn].append( TDT_gh(
                    os.path.join(outputDir, '%s%d' % (peneFunc, sn), 'Linkage', 'Aff_%d' % ch), loci=loci, gh=geneHunter))
            if 'Linkage' in mappingMethods:
                print 'Applying Linkage method to chromosome %d of sample %d' % (ch, sn)
                res['LOD_%d' % sn].append( LOD_gh(
                    os.path.join(outputDir, '%s%d' % (peneFunc, sn), 'Linkage', 'Aff_%d' % ch), loci=loci, gh=geneHunter))
        if hasRPy and 'Association' in mappingMethods:
            print 'Applying chi-sq association tests to sample %d' % sn
            res['ChiSq_%d' % sn] = ChiSq_test(
                os.path.join(outputDir, '%s%d' % (peneFunc, sn), r, 'caseControl.txt'), loci=loci)
    return res
...
if __name__ == '__main__':
    ...
    import pickle
    pickle.dump(res, open(os.path.join(outputDir, '%s.res' % peneFunc), 'w'))
