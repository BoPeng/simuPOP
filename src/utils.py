...
class Trajectory:
    ...
    def plot(self, filename=None, **kwargs):
        '''Plot simulated Trajectory using ``matplotlib``. The function will 
        return silently if module ``matplotlib`` cannot be imported.
        ...
        *endGen* even if the trajectories are short. The y-axis ranges from 0
        ...
        module ``plotter``, with prefix ``figure``, ``plot``, ``set_title``,
        ``set_ylim``, ``set_xlabel``, and ``set_ylabel``, and suffixes ``loc``
        and ``sp``.  '''
