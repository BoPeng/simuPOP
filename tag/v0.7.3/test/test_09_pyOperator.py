...
class TestPyOperator(unittest.TestCase):
    ...
    def testPyIndOperator(self):
        ...
        def indFunc(ind):
            'do something to info1'
            ind.setInfo(ind.info('info1')+1, 'info1')
            return True
        def indFunc1(ind, param):
            'do something to info2'
            ind.setInfo(ind.info('info2')+param[1], 'info2')
            return True
        # use noMating, otherwise info will *not* be inherited
        simu = simulator(population(10, loci=[1], infoFields=['info1', 'info2']), 
            noMating(), rep=1)
        simu.evolve(
            preOps = [initByValue([0])],
            ops = [
                #pyEval(r'"%d\n"%gen'),
                #pyExec('print pop.indInfo("info1")', exposePop=True),
                pyIndOperator(func=indFunc),
                pyIndOperator(func=indFunc1, param=(1,2)),
            ],
            end = 5
        )
        self.assertEqual(simu.pop(0).indInfo('info1'), tuple([6.0]*10))
        self.assertEqual(simu.pop(0).indInfo('info2'), tuple([12.]*10))
