...
# population size
sz = 1000
# SNP or multiallelic marker? maxAllele=2 or 5
ma = 5
try:
  ...
  simu = simulator(population(size=sz, ploidy=2, 
    loci=[31], lociDist=[[x/30.*3 for x in range(0,31)]]),
    randomMating(), rep=1)

  # test for fixaction through alleleNum
  def fixation(an):
    for an_loc in an:
      # if any allele has number 2*sz
      if True in [x==sz*2 for x in an_loc]:
        return True
    return False
  ...
  while True:
    ...
    simu.evolve(
      preOps = [
        ...
        # init MST
        initByValue(values=[[1]*30,[2]*30,[3]*30,[4]*30,[5]*30],
          atLoci=range(1,31), proportions=[.2]*5),
        # init QTL
        initByValue([1], atLoci=[0]),
      ],
      ops = [
        # mutation
        ...
        pointMutator(inds=[0], atLoci=[0], toAllele=2, at = [100]),
        # recombination, with rate 0.001. 
        recombinator(rate=0.001),
        # selective advantage
        basicSelector(locus=0, fitness={'1-1':1, '1-2':1.5, '2-2':2},
          begin = 100, end=110),
        # check fixation
        terminateIf('fixation(alleleNum)', begin = 110),
        #pyEval('gen,alleleFreq[0][1]', begin = 100),
        #endl(rep=REP_LAST, begin = 100),
      ],
      end=200
    )
    if simu.gen() < 200 or fixation(simu.population(0).dvars().alleleNum):
...
for i in range(0,samples):
  # calculate statistics
  q = basicQuanTrait(locus=0, qtrait={'1-1':1, '1-2':1.5, '2-2':2},
    sigma=.1)
  q.apply(validPops[i])
