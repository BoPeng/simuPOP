...
(defvar py-msg-queue nil
  "Queue of executing Python code awaiting execution.
Currently-active file is at the head of the list.")
...
(defun py-execute-file (proc filename msg)
...
This function is appropriate for `comint-output-filter-functions'."
  ...
  (when (and (or (string-equal string ">>> ")
    ...
    (setq py-msg-queue (cdr py-msg-queue))
    (if py-file-queue
	(let ((pyproc (get-buffer-process (current-buffer))))
	  (py-execute-file pyproc (car py-file-queue) (car py-msg-queue) )))
    ))
...
is inserted at the end.  See also the command `py-clear-queue'."
  ...
  (let* ((proc (get-process py-which-bufname))
	...
	(write-region (point-min) (point-max) file nil 'nomsg))
	  ...
	  (py-execute-file proc file text-to-be-executed)
	(message "File %s queued for execution" file))
      (setq py-file-queue (append py-file-queue (list file)))
      (setq py-msg-queue (append py-msg-queue (list text-to-be-executed)))
