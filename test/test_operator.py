...
# load module.
from simuPOP import *
import unitest, os, sys

class TestOperator(unittest.TestCase):

 	def setUp(self):
    self.pop = population(size=10, ploidy=2, loci=[2, 3 ], subPop=[])
    InitByFreq(self.pop, [.2, .3, .5])
    self.dumper = dumper()

  def testMemberFunction(self):
		'member functions of an operator'
    assert not self.dumper.canApplyPreMating()
    assert not self.dumper.canApplyDuringMating()
    # so dumper is a post mating operator
    assert self.dumper.canApplyPostMating()
		
    # apply to any replicate
    assert self.dumper.applicableReplicate() == -1
		# apply to any group of replicates
    assert self.dumper.applicableGroup() == -1

    # an operator can be applied to a population
    self.dumper.apply(self.pop)
		
    # most arguments can be inquired and reset. The name convention is
    # argumentName() and setArgumentName()
    assert self.dumper.alleleOnly()
    self.dumper.setAlleleOnly(0)
    self.dumper.apply(pop)
