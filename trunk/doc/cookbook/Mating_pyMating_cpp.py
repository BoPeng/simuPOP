...
'''
 ...
 choose a female that is geographically closest a random male. A hybrid
 ...
 3. An interface file demoNonRandomMating.i, which tells an automatic wrapper
 ...
 2. generate a wrap using commands like:
 
	> swig -python -templatereduce -c++ -shadow -nodefaultctor  -keyword demoNonRandomMating.i
	A wrapper file demoNonRandomMating_wrap.cxx will be generated. Options -c++
    -shadow are not needed if the header file only contains C/C++ functions.
 
 3. compile the wrapper, and your .cpp files (if they are available)
...
try:
    from demoNonRandomMating_cpp import *
    has_cpp_chooser = True
except:
    print "Module demoNonRandomMating_cpp can not be loaded"
