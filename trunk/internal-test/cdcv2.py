...
#
def PlotSpectra(pop, numDSL, saveAt, logFile, highest=50, plot=True ):
  out = open(logFile, 'a')
  out.write("%d\t" % pop.gen())
  for sp in range(pop.numSubPop()):
    ...
    for d in range(numDSL):
      out.write( '%.5f\t%d\t%.5f\t%.5f\t%.5f\t%.5f\t' % (effNumAllele[d], \
...
def specPlotter(numDSL, saveAt, logFile,  plot=True, **kwargs):
  ...
  cmd = '''pyExec(exposePop=1, %s
    stmts="PlotSpectra(pop, %s, saveAt=%s, logFile='%s', plot=%d)", 
    name="specPlotter" ) ''' \
    % ( parm, numDSL, str(saveAt), logFile, plot) 
  return eval(cmd)
...
def simuCDCV( numDSL, initSpec, selModel,
  ...
  simu.evolve(              # start evolution
    ...
    ops=[     
      ...
      stat(popSize=True, alleleFreq=range(numDSL), at=[burnin]+[i*update for i in range(end/update)]),
      ...
      migration, 
      # visualizer
      specPlotter(numDSL, saveAt=saveAt, logFile=logFile, plot=dispPlot, step=update),
