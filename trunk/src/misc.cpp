                            ...
                            bool restartIfFail, long maxAttempts, bool allowFixation)
{
	...
	vectori Ntmp(1, N);
	if (NtFunc.isValid()) {
		Ntmp = NtFunc(PyObj_As_IntArray, "(i)", curGen);
	...
	while (true) {
		...
		if (idx + 1 >= Nt.size() ) {
			if (NtFunc.isValid()) {
				Ntmp = NtFunc(PyObj_As_IntArray, "(i)", curGen - idx - 1);
				DBG_ASSERT(Ntmp.size() >= 1, ValueError,
					"Return value from NtFunc should be an array of size >= 1");
				// Ntmp[0] will be the total size.
				for (size_t i = 1; i < Ntmp.size(); ++i)
		...
		if (fitnessFunc.isValid()) {
			if (idx + 1 >= s1_cache.size() ) {
				s_vec = fitnessFunc(PyObj_As_Array, "(i)", curGen - idx - 1);
                                ...
                                bool restartIfFail, long maxAttempts)
{
	...
	while (true) {
		...
		if (fitnessFunc.isValid()) {
			...
			sAllTmp = fitnessFunc(PyObj_As_Array, "(iO)", curGen - idx - 1, freqObj);
                             ...
                             long maxAttempts)
{
	...
	for (idx = curGen; idx <= endGen; ++idx) {
		...
		if (NtFunc.isValid()) {
			...
			Ntmp = NtFunc(PyObj_As_IntArray, "(iO)", idx, lastSize);
			Py_XDECREF(lastSize);
		...
		for (size_t sp = 0; sp < nSP; ++sp) {
			...
			if (fitnessFunc.isValid()) {
				...
				PyObject * freqObj = Double_Vec_As_NumArray(a_frq[sp].begin(),
					a_frq[sp].end());
				sAllTmp = fitnessFunc(PyObj_As_Array, "(iO)", idx, freqObj);
