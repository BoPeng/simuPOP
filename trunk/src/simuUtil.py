...
try:
  ...
  def listVars(var, level=-1, name='', subPop=True):
    ''' 
    ...
    if level < 0 or (level > 0 and curLevel < level):
      if type(var) == types.ListType or type(var) == types.TupleType:
        ...
        for x in var:
          ...
          if type(x) == types.ListType or type(x) == types.DictType:
        ...
        # none array first
        for x in var.items():
          if type(x[1]) != types.ListType and type(x[1]) != types.DictType:
            if name == '' or x[0] == name:
              print ' '*indent, x[0], ':\t', x[1]
        # array but not subPop
        for x in var.items():
          if x[0] != 'subPop' and type(x[1]) == types.ListType or type(x[1]) == types.DictType:
            if name == '' or x[0] == name:
              print ' '*indent, x[0], ':\n',         
...
# a wxPython version of listVars
  ...
  import wx.py.filling as fill
  def listVars(var=None, level=-1, name='var', subPop=True):
    ''' 
      var:    any variable to be viewed. Can be a dw object returned
              by dvars() function
      level:  level of display.
      name:   only view certain variable
      subPop: whether or not display info in subPop
    level, subPop will be ignored when wxPython is available.
    '''
    app = wx.App()
...
# functions used to test simuPOP expressions.
# evaluate given expression in main namespace
# compare the result with res
def testExpr(expr='', stmts='', res='', excpt='', d={}):
  try:
    if stmts != '':
      exec stmts in d
    if expr != '':
      if eval(expr, d) != res:
        raise exceptions.ValueError("Expect result for '" + expr \
          + "' is " + str(res) + ".\n  Returned value: " + str(eval(expr, d)))
  except exceptions.Exception, inst:
    if( excpt != '' ):
      try:
        raise inst
      except excpt:
        pass
         ...
         raise exceptions.ValueError("Expecting an exception for '" + expr)
    else:
      print "Error message: \n\n  ", inst, "\n"
      raise exceptions.ValueError("Expecting result " +  str(res))
  else:  # if no exception
    if excpt != '':
      raise exceptions.ValueError("Expected exception is not raised.")
