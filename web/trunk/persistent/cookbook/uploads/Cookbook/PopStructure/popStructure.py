...
def calcFst(pop):
    ...
    sample = RandomSample(pop, size=[500, 500, 500])[0]
    Stat(sample, structure=range(5), vars=['F_st', 'G_st'])
    print 'Gen: %3d Gst: %.3e (all), %.3e (sample) Fst: %.3e (all) %.3e (sample)' \
        % (pop.dvars().gen, pop.dvars().G_st, sample.dvars().G_st,
           pop.dvars().F_st, sample.dvars().F_st)
    return True

simu = simulator(population([10000]*3, loci=[1]*5, infoFields='migrate_to'),
    randomMating())
simu.evolve(
    preOps = [
